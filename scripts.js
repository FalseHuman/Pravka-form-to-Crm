(function ($) {
	...
	wpcf7.submit = function (form) {
		...
		for (let i in detail.inputs) {
			if (detail.inputs[i].name !== 'flp' && detail.inputs[i].name !== 'b24_trace') {
				dict[detail.inputs[i].name] = detail.inputs[i].value
			}
			else if (detail.inputs[i].name === 'flp') {
				str = detail.inputs[i].value
			}
		}
		let result = [str.split(' ', 1).toString(), str.split(' ').slice(1).join(' ')];
		let last_name = result[0]
